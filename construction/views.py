...
def construction_list_call(request):
    return render(request, 'construction_list.html')
        ...
        with transaction.atomic():
            for index, item in enumerate(data_list):
                ...
                if isinstance(raw_id, str) and raw_id.startswith('u'):
                    ...
                    serializer = ConstructionSerializer(data=save_data)
                    instance_exists = False
                else:
                    # 💡 既存データの更新
                    instance = Construction.objects.filter(id=raw_id).first()
                    if not instance:
                        raise ValueError({'error': f'{index + 1}件目のデータ（ID: {raw_id}）がデータベースに存在しません。'})

                    serializer = ConstructionSerializer(instance, data=save_data, partial=True)
                ...
                if serializer.is_valid():
                    ...
                    result_item = ConstructionSerializer(saved_instance).data
