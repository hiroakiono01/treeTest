...
from api.serializers import AggregationSerializer
from app.models import Aggregation


def aggregation_list_call(request):
    return render(request, 'aggregation_list.html')


@api_view(['GET'])
def aggregation_list(request, client_id):
    if request.method == 'GET':
        aggregations = Aggregation.objects.order_by("aggregation_no").exclude(aggregation_name="").exclude(aggregation_name__isnull=True).filter(client_id=client_id).all()
        serializer = AggregationSerializer(aggregations, many=True)
        return Response(serializer.data)


@api_view(["DELETE"])
def aggregation_detail(request, pk):
    try:
        instance = Aggregation.objects.get(pk=pk)
...
@api_view(['POST'])
def bulk_sync_aggregations(request):
        ...
        with transaction.atomic():
            for index, item in enumerate(data_list):
                ...
                if isinstance(raw_id, str) and raw_id.startswith('u'):
                    ...
                    serializer = AggregationSerializer(data=save_data)
                    instance_exists = False
                else:
                    # 💡 既存データの更新
                    instance = Aggregation.objects.filter(id=raw_id).first()
                    if not instance:
                        raise ValueError({'error': f'{index + 1}件目のデータ（ID: {raw_id}）がデータベースに存在しません。'})

                    serializer = AggregationSerializer(instance, data=save_data, partial=True)
                ...
                if serializer.is_valid():
                    ...
                    result_item = AggregationSerializer(saved_instance).data
